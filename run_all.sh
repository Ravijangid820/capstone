...
uv run python -m braintumor_fl.train_centralized --data-root "$DR" $SPLIT --epochs "$EPOCHS" $EXTRA --out data/centralized_unet.pt
echo "### 2/8 centralized per-hospital eval"
uv run python fl/evaluate.py --data-root "$DR" $SPLIT --model data/centralized_unet.pt --method centralized $EXTRA
echo "### 3/8 local-only floor"
uv run python fl/run_local_baselines.py --data-root "$DR" $SPLIT --epochs "$EPOCHS" $EXTRA
echo "### 4/8 FedAvg"
uv run python fl/run_fedavg.py --data-root "$DR" $SPLIT --rounds "$ROUNDS" --epochs 1 --method fedavg $GPUFLAG $EXTRA
echo "### 5/8 FedProx"
uv run python fl/run_fedavg.py --data-root "$DR" $SPLIT --rounds "$ROUNDS" --epochs 1 --method fedprox --prox-mu 0.01 $GPUFLAG $EXTRA
echo "### 6/8 FedBN (primary personalization)"
uv run python fl/run_fedavg.py --data-root "$DR" $SPLIT --rounds "$ROUNDS" --epochs 1 --method fedbn --personalization fedbn $GPUFLAG $EXTRA
echo "### 7/8 fine-tune personalization"
uv run python fl/finetune.py --data-root "$DR" $SPLIT --ft-epochs 5 $EXTRA
